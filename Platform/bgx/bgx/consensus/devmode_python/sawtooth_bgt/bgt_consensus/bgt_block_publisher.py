...
from sawtooth_bgt.state.settings_view import SettingsView
...
from sawtooth_bgt.bgt_consensus.signup_info import SignupInfo
from sawtooth_bgt.bgt_consensus.bgt_key_state_store \
    import BgtKeyState
from sawtooth_bgt.bgt_consensus.bgt_key_state_store \
    import BgtKeyStateStore
from sawtooth_bgt.bgt_consensus.wait_timer import WaitTimer
from sawtooth_bgt.bgt_consensus.wait_certificate import WaitCertificate
from sawtooth_bgt.bgt_consensus import utils

import sawtooth_bgt_common.protobuf.validator_registry_pb2 as vr_pb
...
class BgtBlockPublisher(BlockPublisherInterface):
    ...
    def __init__(self,
        ...
        self._bgt_key_state_store = \
            BgtKeyStateStore(
                data_dir=self._data_dir,
                validator_id=self._validator_id)
        self._wait_timer = None

    def _register_signup_information(self, block_header, bgt_enclave_module):
        ...
        payload = \
            vr_pb.ValidatorRegistryPayload(
                ...
                signup_info=vr_pb.SignUpInfo(
                    bgt_public_key=signup_info.bgt_public_key,
        ...
        validator_entry_address = \
            BgtBlockPublisher._validator_registry_namespace + \
        ...
        output_addresses = \
            [validator_entry_address,
             BgtBlockPublisher._validator_map_address]
        input_addresses = \
            output_addresses + \
        ...
        LOGGER.info(
            ...
            payload.id[:8],
            payload.id[-8:],
            payload.signup_info.bgt_public_key[:8],
            payload.signup_info.bgt_public_key[-8:],
            nonce)
        ...
        LOGGER.info(
            ...
            signup_info.bgt_public_key[:8],
            signup_info.bgt_public_key[-8:],
            signup_info.sealed_signup_data[:8],
            signup_info.sealed_signup_data[-8:])
        self._bgt_key_state_store[signup_info.bgt_public_key] = \
            BgtKeyState(
                sealed_signup_data=signup_info.sealed_signup_data,
                has_been_refreshed=False,
                signup_nonce=nonce)
        self._bgt_key_state_store.active_key = signup_info.bgt_public_key
    ...
    def _handle_registration_timeout(self, block_header, bgt_enclave_module,
        ...
        if consensus_state.signup_attempt_timed_out(
            ...
            del self._bgt_key_state_store[bgt_public_key]
    ...
    def initialize_block(self, block_header):
        ...
        if block_header.previous_block_id == \
                BgtBlockPublisher._previous_block_id:
            return False
        BgtBlockPublisher._previous_block_id = block_header.previous_block_id
        #return True # fake
        ...
        # Get our validator registry entry to see what BGT public key
        ...
        active_bgt_public_key = self._bgt_key_state_store.active_key
        if validator_info is None:
            if active_bgt_public_key is None:
                LOGGER.debug(
                ...
                try:
                    nonce = self._bgt_key_state_store[
                        active_bgt_public_key].signup_nonce
                except (ValueError, AttributeError):
                    self._bgt_key_state_store.active_key = None
                    LOGGER.warning('Bgt Key State Store had inaccessible or '
                                   'corrupt active key [%s] clearing '
                                   'key.', active_bgt_public_key)
                    return False
                ...
                self._handle_registration_timeout(
                    ...
                    state_view=state_view,
                    signup_nonce=nonce,
                    bgt_public_key=active_bgt_public_key
                )
            return False
        ...
        try:
            bgt_key_state = \
                self._bgt_key_state_store[
                    validator_info.signup_info.bgt_public_key]
        ...
        if validator_info.signup_info.bgt_public_key != \
                active_bgt_public_key:
            active_bgt_public_key = validator_info.signup_info.bgt_public_key
            self._bgt_key_state_store.active_key = active_bgt_public_key

        # Ensure that the enclave is using the appropriate keys
        try:
            unsealed_bgt_public_key = \
            ...
            LOGGER.error(
                ...
                active_bgt_public_key[:8],
                active_bgt_public_key[-8:])
            self._bgt_key_state_store.active_key = None
            return False

        assert active_bgt_public_key == unsealed_bgt_public_key
        ...
        if consensus_state.validator_has_claimed_block_limit(
            ...
            # validators.
            bgt_key_state = self._bgt_key_state_store[active_bgt_public_key]
            if not bgt_key_state.has_been_refreshed:
                ...
                sealed_signup_data = bgt_key_state.sealed_signup_data
                signup_nonce = bgt_key_state.signup_nonce
                self._bgt_key_state_store[active_bgt_public_key] = \
                    BgtKeyState(
        ...
        BgtBlockPublisher._previous_block_id = None
    ...
    def finalize_block(self, block_header):
        ...
        if isinstance(block_header, bytes):
            ...
            active_key = self._bgt_key_state_store.active_key
            bgt_key_state = self._bgt_key_state_store[active_key]
            sealed_signup_data = bgt_key_state.sealed_signup_data
            try:
                wait_certificate = \
