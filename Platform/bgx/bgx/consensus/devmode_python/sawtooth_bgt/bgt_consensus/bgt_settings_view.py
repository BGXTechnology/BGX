...
class BgtSettingsView:
    """A class to wrap the retrieval of BGT configuration settings from the
    ...
    _ENCLAVE_MODULE_NAME_ = \
        'sawtooth_bgt_simulator.bgt_enclave_simulator.bgt_enclave_simulator'
    ...
    def __init__(self, state_view):
        """Initialize a BgtSettingsView object.
        ...
        if self._block_claim_delay is None:
            self._block_claim_delay = \
                self._get_config_setting(
                    name='sawtooth.bgt.block_claim_delay',
                    value_type=int,
                    default_value=BgtSettingsView._BLOCK_CLAIM_DELAY_,
    ...
    def enclave_module_name(self):
        ...
        if self._enclave_module_name is None:
            self._enclave_module_name = \
                self._get_config_setting(
                    name='sawtooth.bgt.enclave_module_name',
                    value_type=str,
                    default_value=BgtSettingsView._ENCLAVE_MODULE_NAME_,
    ...
    def initial_wait_time(self):
        ...
        least population_estimate_sample_size BGT blocks in the blockchain.
        """
        if self._initial_wait_time is None:
            self._initial_wait_time = \
                self._get_config_setting(
                    name='sawtooth.bgt.initial_wait_time',
                    value_type=float,
                    default_value=BgtSettingsView._INITIAL_WAIT_TIME_,
    ...
    def key_block_claim_limit(self):
        ...
        validator may claim with a BGT key pair before it needs to refresh
        its signup information.
        """
        if self._key_block_claim_limit is None:
            self._key_block_claim_limit = \
                self._get_config_setting(
                    name='sawtooth.bgt.key_block_claim_limit',
                    value_type=int,
                    default_value=BgtSettingsView._KEY_BLOCK_CLAIM_LIMIT_,
                    validate_function=lambda value: value > 0)
    ...
    def population_estimate_sample_size(self):
        ...
        if self._population_estimate_sample_size is None:
            self._population_estimate_sample_size = \
                self._get_config_setting(
                    name='sawtooth.bgt.population_estimate_sample_size',
                    value_type=int,
                    default_value=BgtSettingsView.
    ...
    def registration_retry_delay(self):
        ...
        if self._registration_retry_delay is None:
            self._registration_retry_delay = \
                self._get_config_setting(
                    name='sawtooth.bgt._registration_retry_delay',
                    value_type=int,
                    default_value=BgtSettingsView.
    ...
    def signup_commit_maximum_delay(self):
        ...
        if self._signup_commit_maximum_delay is None:
            self._signup_commit_maximum_delay = \
                self._get_config_setting(
                    name='sawtooth.bgt.signup_commit_maximum_delay',
                    value_type=int,
                    default_value=BgtSettingsView.
    ...
    def target_wait_time(self):
        ...
        if self._target_wait_time is None:
            self._target_wait_time = \
                self._get_config_setting(
                    name='sawtooth.bgt.target_wait_time',
                    value_type=float,
                    default_value=BgtSettingsView._TARGET_WAIT_TIME_,
                    validate_function=lambda value:
                        math.isfinite(value) and value > 0)
    ...
    def ztest_maximum_win_deviation(self):
        ...
        if self._ztest_maximum_win_deviation is None:
            self._ztest_maximum_win_deviation = \
                self._get_config_setting(
                    name='sawtooth.bgt.ztest_maximum_win_deviation',
                    value_type=float,
                    default_value=BgtSettingsView.
    ...
    def ztest_minimum_win_count(self):
        ...
        population_estimate_sample_size BGT blocks in the blockchain) before
        ...
        if self._ztest_minimum_win_count is None:
            self._ztest_minimum_win_count = \
                self._get_config_setting(
                    name='sawtooth.bgt.ztest_minimum_win_count',
                    value_type=int,
                    default_value=BgtSettingsView._ZTEST_MINIMUM_WIN_COUNT_,
                    validate_function=lambda value: value >= 0)
