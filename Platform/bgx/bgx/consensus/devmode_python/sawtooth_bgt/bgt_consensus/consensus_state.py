...
bgt_public_key (str): The current BGT public key for the validator
...
class ConsensusState:
    ...
    _BlockInfo = \
        collections.namedtuple(
            '_BlockInfo',
            ['wait_certificate', 'validator_info', 'bgt_settings_view'])
    ...
    bgt_settings_view (BgtSettingsView): The BGT settings view associated
        with the block
    """
        ...
        for current_id, block_info in reversed(blocks.items()):
            ...
            # state store.
                ...
                consensus_state.validator_did_claim_block(
                    ...
                    bgt_settings_view=block_info.bgt_settings_view)
    ...
    def _check_validator_state(validator_state):
        ...
        if not isinstance(
                validator_state.bgt_public_key, str) \
                or not validator_state.bgt_public_key:
            raise \
                ValueError(
                    'bgt_public_key ({}) is invalid'.format(
                        validator_state.bgt_public_key))
    ...
    def _build_population_estimate_list(self,
        ...
        Args:
            ...
            bgt_settings_view (BgtSettingsView): The current BGT settings
                view
            block_cache (BlockCache): The block store cache
            bgt_enclave_module (module): The BGT enclave module
    ...
    def _compute_population_estimate(self, bgt_settings_view):
    ...
    def compute_local_mean(self, bgt_settings_view):
        ...
        if self._local_mean is None:
            ...
            if count < population_estimate_sample_size:
                ...
                target_wait_time = bgt_settings_view.target_wait_time
                initial_wait_time = bgt_settings_view.initial_wait_time
                self._local_mean = \
        ...
        if validator_state is None:
            validator_state = \
                ValidatorState(
                    ...
                    bgt_public_key=validator_info.signup_info.
                    bgt_public_key,
    ...
    def signup_attempt_timed_out(self,
        ...
        # an old network.
        depth = bgt_settings_view.registration_retry_delay
    ...
    def validator_signup_was_committed_too_late(self,
        ...
        for _ in range(bgt_settings_view.signup_commit_maximum_delay + 1):
        ...
        LOGGER.info(
            ...
            commit_block_id[:8],
            validator_info.signup_info.nonce,
            bgt_settings_view.signup_commit_maximum_delay + 1)
        return True
    ...
    def validator_has_claimed_block_limit(self,
        ...
        Returns:
            bool: True if the validator has already claimed the maximum
                ...
                otherwise
        """
        key_block_claim_limit = bgt_settings_view.key_block_claim_limit
        validator_state = \
            self.get_validator_state(validator_info=validator_info)

        if validator_state.bgt_public_key == \
    ...
    def validator_is_claiming_too_early(self,
        ...
        block_claim_delay = \
            min(bgt_settings_view.block_claim_delay, number_of_validators - 1)
    ...
    def validator_is_claiming_too_frequently(self,
        ...
        expected_wins = 0
        block_count = 0
        minimum_win_count = bgt_settings_view.ztest_minimum_win_count
        maximum_win_deviation = bgt_settings_view.ztest_maximum_win_deviation
